...
// TestVerifyVSASignature tests the verifyVSASignature function
func TestVerifyVSASignature(t *testing.T) {
	tests := []struct {
		name        string
		vsaContent  string
		publicKey   string
	...
	}{
		{
			...
			vsaContent:  `{"payload": "test", "payloadType": "application/vnd.in-toto+json", "signatures": []}`,
			publicKey:   "nonexistent-key.pem",
			expectError: true,
			errorMsg:    "failed to load verifier from public key file",
		},
		{
			name:        "invalid DSSE envelope JSON",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := verifyVSASignature(tt.vsaContent, tt.publicKey)
