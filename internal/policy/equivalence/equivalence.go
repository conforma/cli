...
func (ec *EquivalenceChecker) normalizeURISet(uris []string) string {
	...
	for _, uri := range uris {
		// Strip digest from URI for comparison
		normalizedURI := ec.stripDigestFromURI(uri)
...
// stripDigestFromURI removes digest from URI for comparison purposes
func (ec *EquivalenceChecker) stripDigestFromURI(uri string) string {
	// Handle OCI URIs with digest (e.g., oci::registry.com/repo:tag@sha256:digest)
	if strings.Contains(uri, "@sha256:") {
		parts := strings.Split(uri, "@sha256:")
		if len(parts) >= 2 {
			// Remove the digest part, keeping only the base URI
			return parts[0]
		}
	}

	// Handle other digest formats if needed (e.g., @sha512:)
	if strings.Contains(uri, "@sha512:") {
		parts := strings.Split(uri, "@sha512:")
		if len(parts) >= 2 {
			return parts[0]
		}
	}

	// Return original URI if no digest found
	return uri
}
